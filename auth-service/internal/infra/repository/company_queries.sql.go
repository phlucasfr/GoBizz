...
const activateCompanyByEmail = `-- name: ActivateCompanyByEmail :one
UPDATE company
SET
  is_active = true,
  updated_at = NOW()
...
func (q *Queries) ActivateCompanyByEmail(ctx context.Context, email string) (Company, error) {
	row := q.db.QueryRow(ctx, activateCompanyByEmail, email)
	var i Company
	err := row.Scan(
		&i.ID,
		&i.Name,
